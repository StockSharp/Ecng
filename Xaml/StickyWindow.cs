...
namespace Ecng.Xaml
{
	...
	public class StickyWindow : System.Windows.Forms.NativeWindow
	{
		...
		/// <see cref="RegisterExternalReferenceForm(Ecng.Xaml.IFormAdapter)"/>
		/// </summary>
				...
				case WM.NCLBUTTONDOWN:
					{
						...
						_mousePoint.X = (short)((int)m.LParam).LoWord();
						_mousePoint.Y = (short)((int)m.LParam).HiWord();
		...
		private bool ResizeMsgProcessor(ref Message m)
		{
			...
			switch ((WM)m.Msg)
			{
				...
				case WM.KEYDOWN:
					{
						if ((int)m.WParam == WinApi.VK.VK_ESCAPE)
						{
