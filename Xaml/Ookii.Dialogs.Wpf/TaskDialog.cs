...
namespace Ookii.Dialogs.Wpf
{
    ...
    public partial class TaskDialog : Component, IWin32Window
    {
        ...
        {
            if( !IsDialogRunning )
				throw new InvalidOperationException(Ecng.Xaml.Properties.Resources.TaskDialogNotRunningError);
        ...
        private TaskDialogButton ShowDialog(IntPtr owner)
        {
            if( !OSSupportsTaskDialogs )
				throw new NotSupportedException(Ecng.Xaml.Properties.Resources.TaskDialogsNotSupportedError);

            if( IsDialogRunning )
				throw new InvalidOperationException(Ecng.Xaml.Properties.Resources.TaskDialogRunningError);

            if( _buttons.Count == 0 )
				throw new InvalidOperationException(Ecng.Xaml.Properties.Resources.TaskDialogNoButtonsError);
        ...
        private List<NativeMethods.TASKDIALOG_BUTTON> SetupButtons()
        {
            ...
            {
                if( button.Id < 1 )
					throw new InvalidOperationException(Ecng.Xaml.Properties.Resources.InvalidTaskDialogItemIdError);
                ...
                if( button.ButtonType == ButtonType.Custom )
                {
                    if( string.IsNullOrEmpty(button.Text) )
						throw new InvalidOperationException(Ecng.Xaml.Properties.Resources.TaskDialogEmptyButtonLabelError);
        ...
        private void CheckCrossThreadCall()
        {
            ...
            if( handle != IntPtr.Zero )
            {
                ...
                if( windowThreadId != threadId )
					throw new InvalidOperationException(Ecng.Xaml.Properties.Resources.TaskDialogIllegalCrossThreadCallError);
            }
        }
