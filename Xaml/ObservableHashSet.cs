namespace Ecng.Xaml
{
	...
	/// </summary>
	/// <remarks>http://geoffcox.bellacode.com/2011/12/09/observablehashset/</remarks>
	...
	public sealed class ObservableHashSet<T> : ISet<T>, INotifyCollectionChanged, INotifyPropertyChanged, IDisposable
	{
		...
		public bool Add(T item)
		{
			...
			if (wasAdded)
			{
				...
				RaiseCollectionChanged(new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Add, item, -1));
		...
		{
			//int index = _hashSet.IndexOf(item);
			var wasRemoved = _hashSet.Remove(item);

			if (wasRemoved)
			{

				RaiseCollectionChanged(new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, item, -1));
				RaisePropertyChanged(PropertyNames.Count);
			}
