  ...
  public abstract partial class TreeViewBase<T> : UserControl where T:class
  {
    ...
    protected internal void CreateItemNode(T item, IList parentNodes, TreeLayout layout)
    {
      ...
      else if (hasChilds)
      {
        ...
        treeNode.Items.Add(dummyNode);

        //if we have an active filter, the dummy node might be filtered if
        //none of the substituted childs would pass the filter
        if (ItemFilter != null) ApplyFilterOnDummyNode(treeNode, item, dummyNode);
      }
    ...
    {
      //expand parent items
      TreeViewItem itemNode = EnsureNodeIsVisible(item);
    ...
    protected virtual TreeViewItem EnsureNodeIsVisible(T item)
    {
      ...
      ItemCollection items = GetRootItemNodes();
    ...
    protected virtual void ClearFilter()
    {
      ...
      //by the filter!
      ItemCollection nodes = GetRootItemNodes();
    ...
    public virtual void FilterBranch(TreeViewItem treeNode)
    {
      ...
      if (TreeUtil.ContainsDummyNode(treeNode))
      {
        //if the item is collapsed and contains a dummy node, we have to
        //evaluate whether all items that are substituted by the dummy would
        //get filtered out or not
        TreeViewItem dummyNode = (TreeViewItem)treeNode.Items[0];
        ApplyFilterOnDummyNode(treeNode, item, dummyNode);
      }
      else
      {
    ...
    protected internal virtual bool ApplyFilter(TreeViewItem treeNode, T item)
    {
      bool filterStatus = ItemFilter == null ? true : ItemFilter(item);
      treeNode.Visibility = filterStatus ? Visibility.Visible : Visibility.Collapsed;
      return filterStatus;
    }


    /// <summary>
    /// Determines whether a dummy node should be "filtered" or not. If lazy loading is
    /// enabled, a dummy is used to render an expander on a collapsed regular node in
    /// order to indicate that child items are available.<br/>
    /// In a filtering scenario, applying the filter on the dummy node means that the
    /// node should be hidden if none of the substituted child items would pass the filter,
    /// which means that no expander should be rendered.<br/>
    /// This default implementation simply iterates through the available child items,
    /// and invokes <see cref="ApplyFilter"/> for every item, passing the <paramref name="dummyNode"/>.
    /// As soon as an item passes the filter, the method exits, thus having the parent node
    /// render an expander.
    /// </summary>
    /// <param name="parentNode">The currently processed tree node which should show or hide
    /// its expander based on the number and filter compliance of its child items.</param>
    /// <param name="item">The item that is being represented by <paramref name="parentNode"/>.
    /// This parameter might be null in case of custom-injected tree nodes that were not
    /// created by the tree, but added by custom code.</param>
    /// <param name="dummyNode">The dummy node that was used to ensure that an expander is
    /// being rendered for the tree.</param>
    protected internal virtual void ApplyFilterOnDummyNode(TreeViewItem parentNode, T item, TreeViewItem dummyNode)
    {
      if (item == null)
      {
        //the processed node wasn't created by us - apply default strategy for this
        //scenario and return
        ApplyFilter(dummyNode, null);
        return;
      }
      ...
      {
        //the dummy node will be in a valid state as soon as one
        //of the childs passes the filter, which means we can quit.
        //if all childs fail, we looped through all childs, and
        //the dummy node will have the correct (filtered) setting
        //as well
        bool status = ApplyFilter(dummyNode, childItem);
        if (status) break;
      }
    }
    ...
    protected virtual bool IsDummyNode(TreeViewItem treeNode)
    {
      ...
      return treeNode.Header == null && !ReferenceEquals(treeNode, RootNode);
    }
    ...
    /// <summary>
    /// Gets the top-level nodes that represent the bound
    /// <see cref="Items"/>. This tree node collection is
    /// either directly under the tree, or the child collection
    /// of a custom <see cref="RootNode"/>.
    /// </summary>
    /// <returns>Child collection of the tree or the virtual
    /// <see cref="RootNode"/>.</returns>
    public ItemCollection GetRootItemNodes()
    {
      return RootNode == null ? Tree.Items : RootNode.Items;
    }
  }
}