...
public class SqlDialectTests : BaseTestClass
{
	...
	/// <summary>
	/// Leaving the primary key unnamed lets the database invent one (<c>PK__Table__hash</c> on
	/// SQL Server, <c>table_pkey</c> on PostgreSQL), which no naming convention can match.
	/// The named form must place the CONSTRAINT clause where each dialect accepts it and
	/// keep the auto-increment behaviour intact — notably SQLite, where AUTOINCREMENT is
	/// only legal on a column-level INTEGER PRIMARY KEY.
	/// </summary>
	[TestMethod]
	[DataRow("SqlServer", "IDENTITY(1,1) CONSTRAINT [PK_Order] PRIMARY KEY")]
	[DataRow("SQLite", "CONSTRAINT \"PK_Order\" PRIMARY KEY AUTOINCREMENT")]
	[DataRow("PostgreSql", "GENERATED BY DEFAULT AS IDENTITY CONSTRAINT \"PK_Order\" PRIMARY KEY")]
	public void GetIdentityColumnSuffix_Named(string dialectName, string expected)
	{
		GetDialect(dialectName)
			.GetIdentityColumnSuffix(SchemaNaming.PrimaryKey("Order"))
			.AssertEqual(expected);
	}

	/// <summary>
	/// The named and unnamed forms must describe the same column — same auto-increment
	/// mechanism, same PRIMARY KEY — so naming the constraint cannot change semantics.
	/// </summary>
	[TestMethod]
	[DataRow("SqlServer")]
	[DataRow("SQLite")]
	[DataRow("PostgreSql")]
	public void GetIdentityColumnSuffix_NamedKeepsSameClauses(string dialectName)
	{
		var dialect = GetDialect(dialectName);

		var unnamed = dialect.GetIdentityColumnSuffix();
		var named = dialect.GetIdentityColumnSuffix(SchemaNaming.PrimaryKey("Order"));

		named.Contains("PRIMARY KEY").AssertTrue($"Named form lost PRIMARY KEY: {named}");
		named.Contains("CONSTRAINT").AssertTrue($"Named form lost CONSTRAINT: {named}");

		// Everything the unnamed form said must survive, in order, with only the
		// CONSTRAINT clause inserted.
		named.Remove($"CONSTRAINT {Quote(dialectName, "PK_Order")} ").AssertEqual(
			unnamed, $"Named form changed more than the constraint clause: {named}");
	}
