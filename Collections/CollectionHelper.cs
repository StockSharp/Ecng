...
public static class CollectionHelper
{
	...
	{
		var (val, _) = await dictionary.SafeAddAsyncEx(sync, key, handler, cancellationToken);
		return val;
	}
	...
	/// <exception cref="ArgumentNullException">Thrown when <paramref name="dictionary"/>, <paramref name="sync"/>, or <paramref name="handler"/> is null.</exception>
	[CLSCompliant(false)]
	public static async Task<(TValue val, bool isNew)> SafeAddAsyncEx<TKey, TValue>(this IDictionary<TKey, TValue> dictionary, AsyncReaderWriterLock sync, TKey key, Func<TKey, CancellationToken, Task<TValue>> handler, CancellationToken cancellationToken)
	{
		if (dictionary is null)
			throw new ArgumentNullException(nameof(dictionary));
