...
{
    /// <summary>
    /// The DefaultLabelFormatter is a pass-through which uses the <see cref="AxisBase.TextFormatting"/> and <see cref="AxisBase.CursorTextFormatting"/> properties
    /// to format axis and cursor label texts
    /// </summary>
    public class DefaultLabelFormatter : LabelFormatterBase
    {
        public override ITickLabelViewModel GetFormattedLabel(IComparable dataValue)
        {
            var formatted = FormatLabel(dataValue);

            var label = new NumericTickLabelViewModel { Text = formatted, Significand = formatted, Exponent = "", Separator = "" };

            var numericAxis = ParentAxis as NumericAxis;
            ...
            if (numericAxis != null)
            {
                ...
                if (numericAxis.ScientificNotation != ScientificNotation.None)
                {
                    ...
                    label.Significand = formatted.Substring(0, ind);
                    label.Exponent = formatted.Substring(ind + 1);
                    label.Separator = numericAxis.ScientificNotation == ScientificNotation.Normalized ? "x10" : formatted[ind].ToString(CultureInfo.InvariantCulture);
                }
            }
