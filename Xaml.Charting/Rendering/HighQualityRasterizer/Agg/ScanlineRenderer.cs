...
namespace MatterHackers.Agg
{
    internal class ScanlineRenderer
    {
        ...
        // <param name="getColorCb">callback used tp get color from texture brush. arguments of callback are X and Y of pixel at screen</param>
        public void render_scanlines_aa_solid(IImageByte destImage, IRasterizer rasterizer, IScanlineCache scanLine, Func<int, int, RGBA_Bytes> getColorCb)
        {
            if (rasterizer.rewind_scanlines())
            {
                ...
                {
                    RenderSolidSingleScanLine(destImage, scanLine, getColorCb);
                }
            }
        }
        ...
        protected virtual void RenderSolidSingleScanLine(IImageByte destImage, IScanlineCache scanLine, Func<int, int, RGBA_Bytes> getColorCb)
        {
            ...
            for (; ; )
            {
                ...
                if (scanlineSpan.len > 0)
                {
                    destImage.blend_solid_hspan(x, y, scanlineSpan.len, getColorCb, ManagedCoversArray, scanlineSpan.cover_index);
                }
                else
                {
                    int x2 = (x - (int)scanlineSpan.len - 1);
                    destImage.blend_hline(x, y, x2, getColorCb, ManagedCoversArray[scanlineSpan.cover_index]);
                }
                if (--num_spans == 0) break;
                scanlineSpan = scanLine.GetNextScanlineSpan();
            }
