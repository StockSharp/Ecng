...
namespace TheArtOfDev.HtmlRenderer.Core.Dom
{
    ...
    internal sealed class CssBoxFrame : CssBox
    {
            ...
            : base(parent, tag)
        {
            ...
            if (Uri.TryCreate(GetAttribute("src"), UriKind.Absolute, out uri))
            {
                if (uri.Host.IndexOf("youtube.com", StringComparison.InvariantCultureIgnoreCase) > -1)
                {
					...
					_videoImageUrl = $"https://img.youtube.com/vi/{uri.Segments[2]}/0.jpg";
					_videoLinkUrl = $"https://www.youtube.com/watch?v={uri.Segments[2]}";

					// TODO better use YouTube API
		...
		private void LoadVimeoDataAsync(Uri uri)
        {
            Task.Run(async () =>
            {
                try
                {
					...
					OnDownloadVimeoApiCompleted(await client.GetStringAsync(apiUri, cts.Token));
                }
                catch (Exception ex)
                {
		...
		/// <param name="refresh">is the callback was called async to load image call</param>
		private void OnLoadImageComplete(RImage image, RRect rectangle, bool refresh)
        {
