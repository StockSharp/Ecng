...
{
	/// <summary>
	/// Represents a hash-chain match finder with 3 input bytes.
	/// </summary>
	internal sealed class HashChain3 : IMatchFinder
	{
		#region Constants
		...
		public HashChain3(SlidingWindow window, uint maxDistance, uint minLength)
		{
			this.window = window;
			...
			this.hashTable = new uint[1 << 22]; // 22 bit hash table.
		...
		public void Insert(uint numBytes)
		{
			while (numBytes-- > 0)
			{
				...
				this.currentHash = ((this.currentHash << 7) ^ this.window.buffer[this.myPosition]) & 0x1FFFFFu;

				// update position links.
				uint newPosition = this.myPosition >= 2 ? this.myPosition - 2u : this.window.bufferSize + this.myPosition - 2u;
		...
		public void FindMatches(uint position, uint maxLength, MatchCallback callback)
		{
			...
			int hash;
			if (position < this.window.bufferSize - 2)
				hash = (this.window.buffer[position] << 14) ^ (this.window.buffer[position + 1] << 7) ^ (this.window.buffer[position + 2]);
			else if (position == this.window.bufferSize - 2)
				hash = (this.window.buffer[position] << 14) ^ (this.window.buffer[position + 1] << 7) ^ (this.window.buffer[0]);
			else
				hash = (this.window.buffer[position] << 14) ^ (this.window.buffer[0] << 7) ^ (this.window.buffer[1]);
