namespace Ecng.Interop
{
	...
	public static class WindowsIdentityManager
	{
		...
		private static extern bool LogonUser(string lpszUsername, string lpszDomain, string lpszPassword, LogonType dwLogonType, LogonProvider dwLogonProvider, out SafeAccessTokenHandle phToken);

		[DllImport("kernel32.dll", SetLastError = true)]
		private static extern bool CloseHandle(SafeAccessTokenHandle handle);

		[DllImport("advapi32.dll", SetLastError = true)]
		private static extern bool DuplicateToken(SafeAccessTokenHandle existingTokenHandle, SecurityImpersonationLevel level, out SafeAccessTokenHandle duplicateTokenHandle);

		public static void Login(string name, SecureString password, Action action)
		{
			Login(name, string.Empty, password, action);
		}

		public static void Login(string name, string domain, SecureString password, Action action)
		{
			Login(name, domain, password, LogonType.Network, action);
		}

		public static void Login(string name, string domain, SecureString password, LogonType logonType, Action action)
		{
			if (action is null)
				throw new ArgumentNullException(nameof(action));

			if (!LogonUser(name, domain, password.UnSecure(), logonType, LogonProvider.Default, out var token))
				throw new Win32Exception("Logon user error.");
			
			try
			{
				// mika: seems in .NET CORE no need duplicate anymore
				//// https://stackoverflow.com/a/6007001
				//if (!DuplicateToken(token, SecurityImpersonationLevel.SecurityImpersonation, out var tokenDuplicate))
				//	throw new Win32Exception("Duplicate token error.");
			...
			finally
			{
				if (!CloseHandle(token))
					throw new Win32Exception("Close handle error.");
			}
		}
