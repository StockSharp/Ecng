...
namespace Community.Imaging.Decoders.Jpeg
{
    internal class JPEGFrame
    {
        ...
        {
            var comp = Scan.GetComponentById(componentID);
        ...
        public void DecodeScanBaseline(byte numberOfComponents, byte[] componentSelector, int resetInterval,
                                       JPEGBinaryReader jpegReader, ref byte marker)
        {
            // Set the decode function for all the components
            for (var compIndex = 0; compIndex < numberOfComponents; compIndex++)
            {
                var comp = Scan.GetComponentById(componentSelector[compIndex]);
        ...
        private void DecodeScan(byte numberOfComponents, byte[] componentSelector, int resetInterval,
                                JPEGBinaryReader jpegReader, ref byte marker)
        {
            //TODO: not necessary
            ...
            var lastPosition = jpegReader.BaseStream.Position;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    var interleaved = !(numberOfComponents == 1);

                    if(!interleaved)
                    {
                        var comp = Scan.GetComponentById(componentSelector[0]);

                        comp.SetBlock(mcuIndex);

                        comp.DecodeMCU(jpegReader, h, v);

                        var mcus_per_line = mcus_per_row(comp);
                        var blocks_per_line = (int) Math.Ceiling((double) Width / (8 * comp.factorH));
                    ...
                    else // Components are interleaved
                    {
                        for (var compIndex = 0; compIndex < numberOfComponents; compIndex++)
                        {
                            ...
                            for (var j = 0; j < comp.factorV; j++)
                            {
                                for (var i = 0; i < comp.factorH; i++)
                                {
        ...
        public void DecodeScanProgressive(byte successiveApproximation, byte startSpectralSelection,
                                          byte endSpectralSelection,
                                          byte numberOfComponents, byte[] componentSelector, int resetInterval,
                                          JPEGBinaryReader jpegReader, ref byte marker)
        {
            ...
            var dcOnly = startSpectralSelection == 0;
            var refinementScan = (successiveHigh != 0);
