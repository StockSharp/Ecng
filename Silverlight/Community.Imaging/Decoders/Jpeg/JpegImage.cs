...
namespace Community.Imaging.Decoders.Jpeg
{
    ...
    public class Image
    {
        ...
        public Image ChangeColorSpace(ColorSpace cs)
        {
            ...
            if(_cm.colorspace == ColorSpace.YCbCr && cs == ColorSpace.RGB)
            {
                ...
                for (var x = 0; x < width; x++)
                {
                    ...
                    {
                        ycbcr[0] = _raster[0][x, y]; // 0 is LUMA
                        ycbcr[1] = _raster[1][x, y]; // 1 is BLUE
                        ycbcr[2] = _raster[2][x, y];
        ...
        #region Nested type: ConvertColor

        private delegate void ConvertColor(byte[] colorIn, byte[] colorOut);
