    ...
    public class BBCodeParser
    {
        ...
        public virtual SequenceNode ParseSyntaxTree(string bbCode)
        {
            ...
            while (stack.Count > 1) //close all tags that are still open and can be closed implicitly
            {
                ...
                if (node.Tag.RequiresClosingTag && ErrorMode == ErrorMode.Strict) throw new BBCodeParsingException("The tag {0} was not closed correctly.".Put(node.Tag.Name));
            }
            ...
            while (end < input.Length)
            {
                ...
                if (input[end] == ']' && !escapeFound)
                {
                    if (ErrorMode == ErrorMode.Strict)
                        throw new BBCodeParsingException("The character ']' cannot be used in text or code without beeing escaped. Please write '\\]' instead.");
                }
                ...
                else if (escapeFound)
                {
                    ...
                    {
                        if (ErrorMode == ErrorMode.Strict)
                            throw new BBCodeParsingException("The character '\' is used to escape ']' and '['. Please escape it like this '\\'.");
                    }
                    escapeFound = false;
                }
        ...
        bool ErrorOrReturn(string msgKey, params string[] parameters)
        {
			...
			else throw new BBCodeParsingException(string.IsNullOrEmpty(msgKey) ? "" : msgKey);
        }
    }
